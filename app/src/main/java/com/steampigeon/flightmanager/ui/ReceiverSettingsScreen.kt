            ...
            Button(
                ...
                onClick = {
                    if (receiverConfigMessageState == LocatorMessageState.Idle) {
                        ...
                        // Built from the last read-back, changing only this screen's
                        // field.  Name and channel ride in one message but are now
                        // edited on two screens (the channel lives on Channels), so
                        // sending a locally-staged copy of the whole struct would let a
                        // rename quietly revert a channel change made over there.
                        val target = remoteReceiverConfig.copy(
                            deviceName = stagedReceiverConfig.deviceName
                        )
